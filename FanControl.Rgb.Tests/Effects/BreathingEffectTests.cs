...
{
    // Formula: sine = (sin(seconds*Hz*2π) + 1) / 2. At frame=0: seconds=0 → sine=0.5 → midpoint.
    [Fact]
    ...
    // MBV=true: different values produce different rates → different colors at same frame.
    // At frame=100, framerate=30 → seconds≈3.333. Rate = multiplier * 0.25 Hz.
    // value=0:   MinSpeed=1.0 → 0.25 Hz → sine≈0.067
    // value=100: MaxSpeed=3.0 → 0.75 Hz → sine≈0.5
    [Fact]
    public void ModulateByValue_True_DifferentValues_GiveDifferentColors()
    {
        ...
        effectLow.Apply([dev0], "GPU", null, null, 0f, 100, 30, 1f, [buf0]);
        ...
        effectHigh.Apply([dev100], "GPU", null, null, 100f, 100, 30, 1f, [buf100]);
    ...
    public void ModulateByValue_False_Value0_MatchesMaxSpeed()
    {
        ...
        effectFalse.Apply([devFalse], "GPU", null, null, 0f, 100, 30, 1f, [bufFalse]);
        ...
        effectTrue.Apply([devTrue], "GPU", null, null, 100f, 100, 30, 1f, [bufTrue]);
